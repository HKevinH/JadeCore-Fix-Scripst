UPDATE quest_template SET RewardCurrencyCount1 = RewardCurrencyCount1 / 100 WHERE Id IN (31724, 31725);

DELETE FROM spell_script_names WHERE spell_id = 135422;
INSERT INTO spell_script_names VALUE (135422, "spell_gen_reconfigured_remote_shock");
...
(
    ...
    "Salta a la vista que este sherpen lleva bastante tiempo congelado aquí, pero ves un destello de luz procedente de su mochila.",
    "Шерпен замерз и явно находится тут в таком состоянии какое-то время. А что это сверкает в его тюке?",
    "Шерпен замерз и явно находится тут в таком состоянии какое-то время. А что это сверкает в его тюке?"
);
