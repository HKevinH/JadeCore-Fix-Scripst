...
public:
	...
	{
		boss_immerseusAI(Creature* creature) : BossAI(creature, DATA_IMMERSUS), vehicle(creature->GetVehicleKit()), summons(me)
		{
		...
		void Reset()
		{
			...
			if (instance)
				instance->SetData(DATA_IMMERSUS, NOT_STARTED);
		...
		void EnterCombat(Unit* who)
		{
			...
			if (instance)
			{
				...
				instance->SetData(DATA_IMMERSUS, IN_PROGRESS);
			}
		...
		void DamageTaken(Unit* attacker, uint32& damage)
		{
			...
			if (phase == PHASE_IMMERSEUS_NORMAL && damage >= me->GetHealth())
			{
				...
				else // Boss is done.
				{
					...
					if (Creature* lorewalkerCho = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_LORE_WALKER_CHO_01)))
						lorewalkerCho->GetAI()->DoAction(ACTION_LAST_TALK_BEFORE_BOSS);
		...
		void SpellHit(Unit* caster, SpellInfo const* spell)
		{
			...
			if (instance->GetData(DATA_IMMERSUS) == DONE)
				return;
		...
		void EnterEvadeMode()
		{
			...
			if (instance)
			{
				instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_SEEPING_SHA_DAMAGE);
				instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_SHA_SPLASH_DMG);
				instance->SetData(DATA_IMMERSUS, FAIL);
		...
		void JustDied(Unit* killer)
		{
			...
			if (instance)
			{
				...
				instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_SHA_SPLASH_DMG);
				instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me); // Remove.
				instance->SetData(DATA_IMMERSUS, DONE);
			}
		...
		void UpdateAI(uint32 const diff)
		{
			...
			if (!me->FindNearestCreature(CREATURE_LESSER_SHA_PUDDLE, 200.0f, true) && !me->FindNearestCreature(CREATURE_CONTAMINATED_PUDDLE, 200.0f, true) && secondPhaseInProgress)
			{
				secondPhaseInProgress = false;
				Talk(ANN_REFORM);
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_PUDDLES_MOVE_CENTER:
				{
					...
					GetCreatureListWithEntryInGrid(shaContaminatedPuddleList, me, CREATURE_LESSER_SHA_PUDDLE, 150.0f);
					GetCreatureListWithEntryInGrid(shaContaminatedPuddleList, me, CREATURE_CONTAMINATED_PUDDLE, 150.0f);
	...
	struct npc_sha_splash_bolt_immerseusAI : public ScriptedAI
	{
		...
		void DoAction(int32 const action)
		{
			...
			case ACTION_STOP_SPLASH_CHECK:
				...
				if (Creature* Immerseus = me->FindNearestCreature(BOSS_IMMERSUS, 150.0f, true))
		...
		void UpdateAI(uint32 const diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				{
					// Check for players.
					if (Creature* Immerseus = me->FindNearestCreature(BOSS_IMMERSUS, 300.0f, true))
					{
						Map::PlayerList const& PlayerList = me->GetMap()->GetPlayers();
						if (!PlayerList.isEmpty())
						{
		...
		void UpdateAI(uint32 const diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_MOVE_CIRCLE:
				{
					if (Creature* Immerseus = me->FindNearestCreature(BOSS_IMMERSUS, 150.0f, true))
					{
						me->GetMotionMaster()->MoveRotate(15000, ROTATE_DIRECTION_LEFT);
						events.ScheduleEvent(EVENT_MOVE_CIRCLE, 1000);
					}
					break;
				}
				...
				{
					if (Creature* Immerseus = me->FindNearestCreature(BOSS_IMMERSUS, 200.0f, true))
					{
						Map::PlayerList const& PlayerList = me->GetMap()->GetPlayers();
						if (!PlayerList.isEmpty())
						{
...
public:
	...
	struct npc_sha_puddle_immerseusAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 const diff)
		{
			if (Creature* Immerseus = me->FindNearestCreature(BOSS_IMMERSUS, 1.0f, true))
			{
...
public:
	...
	class spell_immerseus_sha_bolt_SpellScript : public SpellScript
	{
		...
		void HandleDummy(SpellEffIndex /*effIndex*/)
		{
			...
			if (caster->ToCreature()->GetEntry() != BOSS_IMMERSUS)
				return;
	...
	class spell_immerseus_congealing_AuraScript : public AuraScript
	{
		...
		void OnPeriodic(constAuraEffectPtr /*aurEff*/)
		{
			...
			if (stackCount == 10 && caster->HealthAbovePct(99) && !caster->HasAura(SPELL_PURIFIED))
			{
				...
				// Set boss corruption to remove to +1.
				if (Creature* Immerseus = caster->FindNearestCreature(BOSS_IMMERSUS, 150.0f, true))
					CAST_AI(boss_immerseus::boss_immerseusAI, Immerseus->AI())->corruptionToRemove += 1;
