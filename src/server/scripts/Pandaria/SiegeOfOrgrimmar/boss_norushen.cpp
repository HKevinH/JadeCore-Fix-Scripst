...
public:
    ...
    bool Execute(uint64 p_CurrTime, uint32 p_Diff)
    {
        ...
        {
            if (Creature* i_Norushen = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_BOSS)))
            {
                if (Creature* i_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_TRIGGER))) ///< Controller, responsible for the boss summoning.
                {
...
public:
    ...
    bool Execute(uint64 p_CurrTime, uint32 p_Diff)
    {
        if (InstanceScript* l_Instance = m_Obj->GetInstanceScript())
        {
            if (Creature* i_Norushen = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_BOSS)))
            {
                if (Creature* i_Cho = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_CHO)))
                {
                    if (Creature* i_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_TRIGGER))) ///< Controller, responsible for the boss summoning.
                    {
    ...
    struct boss_norushenAI : public ScriptedAI
    {
        ...
        void JustReachedHome() override
        {
            ...
            if (m_Instance != nullptr)
            {
                ...
                if (Creature* l_Amagalem = m_Instance->instance->GetCreature(m_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
...
public:
    ...
    struct boss_amagalad_of_corruptionAI : public BossAI
    {
        ...
        void EntranceDoorActivation() ///< Upon wipe
        {
            ...
            {
                // Reset handling - encounter.
                if (GameObject* l_NorushenEntrance = m_Instance->instance->GetGameObject(m_Instance->GetData(eData::DATA_NORUSHEN_ENTRANCE)))
                {
        ...
        void ExitDoorActivation() ///< After boss defeat
        {
            ...
            {
                // Reset handling - encounter.
                if (GameObject* l_PrideEntrance = m_Instance->instance->GetGameObject(m_Instance->GetData(eData::DATA_PRIDE_ENTRANCE)))
                {
        ...
        void JustReachedHome() override
        {
            ...
            {
                // Reset handling - encounter.
                if (Creature* l_Norushen = m_Instance->instance->GetCreature(m_Instance->GetData(eData::DATA_NORUSHEN_BOSS)))
                {
        ...
        void DamageTaken(Unit* p_Attacker, uint32& p_Damage) override
        {
            ...
            if (me->GetHealthPct() <= 50)
            {
                if (!m_FirstFrayed)
                {
                    if (m_Instance != nullptr)
                    {
                        if (Creature* l_Norushen = m_Instance->instance->GetCreature(m_Instance->GetData(eData::DATA_NORUSHEN)))
                        {
                            if (l_Norushen->IsAIEnabled)
    ...
    struct creature_blind_hatred_triggerAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 const p_Diff) override
        {
            ...
            if (m_DamageDiff <= p_Diff)
            {
                ...
                {
                    // Reset handling - encounter.
                    if (Creature* l_Almegalem = m_Instance->instance->GetCreature(m_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                    {
...
public:
    ...
    bool OnGossipHello(Player* p_Player, Creature* p_Creature)
    {
        if (InstanceScript* l_Instance = p_Creature->GetInstanceScript())
        {
            ...
            // Reset handling - encounter.
            if (Creature* l_Almegalem = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
            {
...
public:
    ...
    struct creature_norushen_manifestation_of_corruptionAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (m_Instance != nullptr)
            {
                if (Creature * l_Trigger = m_Instance->instance->GetCreature(m_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
                    ...
                    if (Player* l_Nearest = me->FindNearestPlayer(1.12f, true))
                    {
                        if (AuraPtr l_Aura = l_Nearest->GetAura(eSpells::SPELL_CORRUPTION))
                        {
                            if (me->GetMap())
                            {
                                ...
                                if (me->GetMap()->GetDifficulty() == MAN10_DIFFICULTY)
                                {
                                    int32 bp = l_Aura->GetEffect(0)->GetBaseAmount();
                                    bp += 40;
        ...
        void HandleOnHit()
        {
            ...
            if (GetHitUnit()->HasAura(eSpells::SPELL_CORRUPTION))
            {
                if (AuraPtr l_Aura = GetHitUnit()->GetAura(eSpells::SPELL_CORRUPTION))
                {
                    ...
                    {
                        if (Creature* l_Amagalem = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION))) ///< Controller, responsible for the boss summoning.
                        {
...
public:
    ...
    class spell_norushen_extract_corruption_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& p_Targets)
        {
            ...
            if (Creature* l_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_TRIGGER))) ///< Controller, responsible for the boss summoning.
            {
    ...
    class spell_norushen_blind_hatred_SpellScript : public SpellScript
    {
        ...
        void OnAfterCast()
        {
            ...
            if (InstanceScript* l_Instance = GetCaster()->GetInstanceScript())
            {
                if (Creature* l_Almegalem = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_ALMAGLEM_OF_CORRUPTION)))
                {
                    if (Creature* l_Trigger = l_Instance->instance->GetCreature(l_Instance->GetData(eData::DATA_NORUSHEN_TRIGGER)))
                    {
